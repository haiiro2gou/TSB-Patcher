...
# 未使用idチェック
    execute unless data storage indexstorage:core UnusedID[-1] store result score @s IndexStorageID run scoreboard players add #StorageIDIndex Global 1
    execute unless data storage indexstorage:core UnusedID[-1] run data modify storage indexstorage: data append value {}
    execute if data storage indexstorage:core UnusedID[-1] run function common:lib/array/session/open
    ...
    execute if data storage indexstorage:core UnusedID[-1] run function common:lib/array/sort_ascend
    ...
    execute if data storage indexstorage:core UnusedID[-1] run function common:lib/array/session/close
...
# ストレージ割り当て
    ...
    summon text_display ~ ~ ~ {UUID:[I;0,0,0,1],text:'{"selector":"@e[tag=ProvideStorage,limit=1]"}'}

...
# 要素の数がずれていないかのチェック
    execute if data storage lib: ArrayA[0] unless data storage lib: ArrayB[0] run tellraw @a [{"storage":"global","nbt":"Prefix.ERROR"},{"text":"リストの要素の数が異なります。 / from lib:array/math/add","color":"white"}]
    execute unless data storage lib: ArrayA[0] if data storage lib: ArrayB[0] run tellraw @a [{"storage":"global","nbt":"Prefix.ERROR"},{"text":"リストの要素の数が異なります。 / from lib:array/math/add","color":"white"}]
# 要素がまだあるなら再帰

...
# 乱数取得&範囲設定
    ...
    execute store result score $Length Temporary if data storage indexstorage: _.LatestAttackInfo.DeathMessage[]
    scoreboard players operation $Argument.Index Lib %= $Length Temporary
# 配列操作
    function lib:array/session/open
    data modify storage lib: Array set from storage indexstorage: _.LatestAttackInfo.DeathMessage

...
# 5? クールタイムを秒数に変換
    execute store result score $Cooldown Temporary run data get storage asset:temp Trigger.LocalCooldown
    scoreboard players operation $Cooldown Temporary /= $20 Const
# 5? クールタイムの小数第一位まで求める
    execute store result score $CooldownDecimal Temporary run data get storage asset:temp Trigger.LocalCooldown
...
# 5?ストレージに入れる
    execute store result storage asset:temp LocalCooldownSecond int 1 run scoreboard players get $Cooldown Temporary
    execute store result storage asset:temp LocalCooldownDecimal int 1 run scoreboard players get $CooldownDecimal Temporary
    execute store result storage asset:temp LocalCooldownMinutes int 1 run scoreboard players get $CooldownMinutes Temporary
...
# リセット
    ...
    data remove storage asset:temp LocalCooldownDecimal
    data remove storage asset:temp LocalCooldownMinutes
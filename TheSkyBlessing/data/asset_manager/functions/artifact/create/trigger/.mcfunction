...
# move
    data modify storage asset:temp Trigger set from storage asset:temp Triggers[-1]
...
# 2? 攻撃情報
    execute if data storage asset:temp Trigger.AttackInfo run function asset_manager:artifact/create/trigger/attack_info
# 3? 必要MP
...
# 6? クールタイム
    execute if data storage asset:temp Trigger.LocalCooldown run function asset_manager:artifact/create/trigger/cooltime
# 7? スペシャル☆クールタイム
    execute if data storage asset:temp Trigger.SpecialCooldown run function asset_manager:artifact/create/trigger/special_cooltime

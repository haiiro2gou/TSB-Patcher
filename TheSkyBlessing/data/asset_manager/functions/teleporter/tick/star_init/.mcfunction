...
# グループIDから対象TP先を取得する
    data modify storage asset:teleporter TargetGroupID set from storage indexstorage: _.TeleporterData.GroupID
...
# つながっている星があるのであれば初期化中タグを付与し、星データをプレイヤーに格納する
    execute if score $ActivateStarCount Temporary matches 2.. run function indexstorage:pull/
    execute if score $ActivateStarCount Temporary matches 2.. run data modify storage indexstorage: _.Temp.Teleporters set from storage asset:teleporter FilteredTeleporters

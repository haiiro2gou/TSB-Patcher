...
# context作成
    ...
    data modify storage asset:context PrevField set from storage asset:effect TargetEffect.PrevField
# 各種イベントを呼び出す
...
# ゴミを消す
    ...
    data remove storage asset:effect TargetEffect.PrevField
# 条件を満たしていればエフェクトを消す
...
# リセット
    ...
    data remove storage asset:context PrevField

...
# エフェクトをすでに持っているかを取得する
    data modify storage lib: Array append from storage indexstorage: _.Effects[].ID
...
# 該当IDのデータの取得を試みる
    ...
    data modify storage lib: Array set from storage indexstorage: _.Effects
...
# Effectsから該当IDのデータを消す
    ...
    data modify storage indexstorage: _.Effects set from storage lib: Array
# セッション閉じる

...
# 1tick前のベクトルの大きさの二乗を取得する
    execute store result score $Prv.SquaredMagnitude Temporary run data get storage indexstorage: _.PlayerPosDiffSquaredMagnitude
...
# 前tickの差分が十分な状態(0.05m/tick,1m/sec)でDiffが急に0になったらおそらくPosのパケロスが発生してるので、Posを前tickのPosとDiffから予測する
    ...
    execute if score $Prv.SquaredMagnitude Temporary matches 50.. if score $Cur.SquaredMagnitude Temporary matches ..15 unless score @s PosPacketLossDetectAfterTick matches 2 run data modify storage lib: ArrayB set from storage indexstorage: _.PlayerPosDiff
...
# パケロスが検出されていたらDiffとCurSquaredMagnitudeを古いもので上書きする(即ち、更新が発生しなくなる)
    execute if score $Prv.SquaredMagnitude Temporary matches 50.. if score $Cur.SquaredMagnitude Temporary matches ..15 unless score @s PosPacketLossDetectAfterTick matches 2 run data modify storage player_manager:pos_fix PosDiff set from storage indexstorage: _.PlayerPosDiff
...
# 次tickにデータ渡す
    ...
    data modify storage indexstorage: _.PlayerPosDiff set from storage player_manager:pos_fix PosDiff
    execute store result storage indexstorage: _.PlayerPosDiffSquaredMagnitude int 1 run scoreboard players get $Cur.SquaredMagnitude Temporary
# リセット

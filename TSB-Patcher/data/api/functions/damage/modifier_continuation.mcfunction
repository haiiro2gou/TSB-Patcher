...
# 補正されていないのに呼び出された場合のエラー
    execute unless data storage api: {DamageLibModified:true} run tellraw @a [{"storage":"global","nbt":"Prefix.ERROR"},{"text":"lib:damage/modifier_continuation","color":"gold"},{"text":"が","color":"white"},{"text":"lib:damage/modifier","color":"gold"},{"text":"より前に使用されています。","color":"white"}]
    execute if data storage global {IsProduction:1b} unless data storage api: {DamageLibModified:true} run tellraw @a [{"storage":"global","nbt":"Prefix.ERROR"},{"text":"あなたが製作者ではない場合、Discordサーバーのお問い合わせより報告してください。","color":"white"}]
# 補正フラグを立てる
